...
def matchPOI():
    ...
    data_didi_HEV_location=pd.concat([data_didi_HEV['latitude'],data_didi_HEV['longitude']],axis=1)

    '''airport'''
    match_points_airport=0
    for i in range(airport_location.shape[0]):
        for j in range(data_didi_HEV_location.shape[0]):
            if (data_didi_HEV_location.loc[j,'latitude'].round(decimals=3)==airport_location.loc[i,'latitude'].round(decimals=3))\
                    and (data_didi_HEV_location.loc[j,'longitude'].round(decimals=3)==airport_location.loc[i,'longitude'].round(decimals=3)):
    ...
    for i in range(railway_location.shape[0]):
        for j in range(data_didi_HEV_location.shape[0]):
            if (data_didi_HEV_location.loc[j,'latitude'].round(decimals=3)==railway_location.loc[i,'latitude'].round(decimals=3))\
                    and (data_didi_HEV_location.loc[j,'longitude'].round(decimals=3)==railway_location.loc[i,'longitude'].round(decimals=3)):
