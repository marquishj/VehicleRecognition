...
class trajectoryAnalysis():
    ...
    def divideDay(self,day):
        for didi_id in range(len(self.didi_list)):
            ...
            '''Jiang Linzhi's idea'''
            column_name = data_vehicle.columns[0]
            if column_name == "time":
                data_vehicle['datatime'] = data_vehicle['time']
        ...
        if radius == 1:
            ...
            for i in range(airport_location.shape[0]):
                for j in range(data_vehicle_location.index[0], data_vehicle_location.index[-1]):
                    '''20201025 round 四舍五入有问题 需要用小数截断'''
                    '''另外用break还是continue？'''
...
if __name__ == '__main__':
    ...
    for file_path in file_paths:
        ...
        sys.stdout =Logger('D:\\result_1-20201014-1km\\HJ\\result_{}'.format(file_suffixs[n])+'.txt')
        sys.stdout =Logger('C:\\result_1-20201014-1km\\HJ\\result_{}'.format(file_suffixs[n])+'.txt')
        traAnalysis = trajectoryAnalysis(file_path,list_input)
        # ------------------选择项--------------------
        # -------------根据分析需要选择功能------------
        # 1-处理轨迹匹配POI,半径单位为km(三个档位:0.1,0.5,1)，默认0.5km
