...
class Analysis():
    ...
    def readVehicleID(self):
        vehicle=[]
        vehicleIDs=[]
        df = pd.read_table("F:\\sql data\\classifer_car_data\\分析结果\\2020.10.19\\POI_0.5km\\result_pcetaxitaxi.txt")
    ...
    def readData(self):
        # df = pd.read_table("F:\\sql data\\classifer_car_data\\分析结果\\2020.10.19\\POI_1.0km\\result_didi.txt")
        df = pd.read_table("F:\\sql data\\classifer_car_data\\分析结果\\2020.10.19\\POI_1.0km\\result_pcev.txt")
        return df
...
if __name__ == '__main__':
    ...
    for id in range(len(vehicle)):
        ...
        print('vehicleID: {}'.format(vehicleIDs[id]))
