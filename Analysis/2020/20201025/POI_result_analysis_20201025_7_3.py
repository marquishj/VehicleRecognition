...
class Analysis():
    ...
    def readVehicleID(self):
        vehicle=[]
        vehicleIDs=[]
        df = pd.read_table("F:\\sql data\\classifer_car_data\\分析结果\\2020.10.19\\POI_0.5km\\result_taxi.txt")
    ...
    def readData(self):
        df = pd.read_table("F:\\sql data\\classifer_car_data\\分析结果\\2020.10.19\\POI_1.0km\\result_taxi.txt")
        return df
